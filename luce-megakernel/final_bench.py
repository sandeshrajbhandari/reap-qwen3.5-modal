...
b = dict(
    ...
    beta_buf=torch.empty(S*DN_NUM_V_HEADS, **f32), alpha_buf=torch.empty(S*DN_NUM_V_HEADS, **f32),
...
elif hasattr(full, "model") and hasattr(full.model, "language_model"):
    lm = full.model.language_model
else:
    raise RuntimeError("Could not find text backbone (language_model) on loaded model")

if hasattr(full, "lm_head"):
    head = full.lm_head
elif hasattr(full, "model") and hasattr(full.model, "lm_head"):
    head = full.model.lm_head
else:
    raise RuntimeError("Could not find lm_head on loaded model")
...
# Warmup 5x
with torch.no_grad():
    for _ in range(5): _ = lm(input_ids); torch.cuda.synchronize()

# PP benchmark (10 runs)
with torch.no_grad():
    torch.cuda.synchronize(); t0 = time.perf_counter()
    for _ in range(10): _ = lm(input_ids); torch.cuda.synchronize()
...
with torch.no_grad():
    ...
    next_id = head(out.last_hidden_state[:, -1:, :]).argmax(-1)
    pt_out_ids = [next_id.item()]

    torch.cuda.synchronize(); t0 = time.perf_counter()
    for _ in range(128):
        out = lm(next_id, past_key_values=past, use_cache=True)
        past = out.past_key_values
        next_id = head(out.last_hidden_state).argmax(-1)
...
# ============================================================
print(f"\n{'='*60}")
print(f"FINAL RESULTS — Qwen3.5-9B BF16, RTX 3090")
print(f"{'='*60}")
