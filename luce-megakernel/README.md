...
<p align="center">
  ...
  This tree targets <strong>Qwen3.5-9B</strong> (32 layers, 4096 hidden): same fused decode + cuBLAS prefill approach as the original 0.8B write-up.<br/>
...
**The culprit: ~100 kernel launches per token.** Each layer boundary returns control to the CPU, dispatches the next kernel, re-fetches weights from global memory, and synchronizes threads. For 32 layers, those microseconds add up, and each one burns power doing nothing useful.

So we fused everything into one kernel.

## Results

### Decode and Prefill (Qwen 3.5-9B in this fork)
...
A single persistent CUDA kernel processes the entire Qwen 3.5-9B **text** forward pass in one dispatch. No CPU round-trips between layers.

**Architecture:** Qwen 3.5-9B text is a hybrid model: 24 linear-attention (DeltaNet-style) layers and 8 full-attention layers in a 3:1 pattern over 32 layers, with 16 query heads / 4 KV heads in full attention and 16 key heads / 32 value heads in linear attention (Q/K broadcast to value heads per Hugging Face). Vision weights in the public checkpoint are not used.
...
| File | Description |
|------|-------------|
| `kernel.cu` | Decode megakernel, all 32 text layers in one dispatch |
...
- **Single model, single architecture.** The kernel is hand-written for Qwen 3.5-9B text (24 linear-attention + 8 full-attention layers). It does not generalize to other checkpoints without editing `kernel.cu`, `prefill.cu`, and `model.py`.
...
- **9B parameters.** Expect higher VRAM use and more compute-bound decode than the original 0.8B article; fusion still removes per-layer launch overhead.
