...
class Collector():
    ...
    def __init__(self, multiplier, head, num_heads: int, head_dim: int):
        self.head = head
    ...
    def __call__(self, b, s):
        if self.head == -1:
            ...
            # b: (batch, seq, num_heads * head_dim) at o_proj input
            self.states.append(
                b[0, -1].reshape(self.num_heads, self.head_dim)[self.head].detach().clone()
            )
            # print('b:', b.shape)
        return b
...
def obtain_act(model, tokenizer, samples):
    ...
    head_dim = getattr(
        model.config, "head_dim", model.config.hidden_size // head_num
    )
    collectors = []
    pv_config = []
    n_heads = model.config.num_attention_heads
    h_dim = getattr(model.config, "head_dim", model.config.hidden_size // n_heads)
    ...
    for s in tqdm(samples):
        ...
        layer_wise_activations, head_wise_activations, _ = get_llama_activations_pyvene(
            collected_model, collectors, sen_ids, device=str(dev)
        )
...
def obtain_main_vecs(data_name,data_num,original_model_path=None,pruned_model_path=None,comp_path = '',score_path = ''):
    ...
    for i in kinds:
        ...
        lid = flat // num_heads
        aid = flat % num_heads
        assert 0 <= lid < num_layers and 0 <= aid < num_heads, (flat, lid, aid, num_layers, num_heads)
