...
- **`quantize_qwen36_mtp_q3ks_modal.py`**: ik_llama.cpp-based Q3_K_S requantizer for `Radamanthys11/Qwen3.6-27B-MTP-Q8_0-GGUF`, preserving `blk.64.*` MTP/nextn tensors as `Q8_0` while the global `Q3_K_S` type handles normal tensors, building the required CPU `llama-quantize` tool without a GPU, and uploading to the authenticated Hugging Face account by default. This path does not generate or use an imatrix, enables `hf_transfer` for Hub metadata, downloads only GGUF files via resumable multi-connection `aria2c`, and does not request any Modal GPU resources.
