"""Prune openai/gpt-oss-20b using REAP on Modal and upload to HF.

Usage:
    modal run prune_gpt_oss.py --hf-repo-id your-username/gpt-oss-20b-reap-pruned
"""
...
REAP_BRANCH = "fix/gpt-oss-smoke-test-and-observer"

DEFAULT_COMPRESSION_RATIO = 0.32
DEFAULT_SAMPLES_PER_CATEGORY = 128
PRUNE_METHOD = "reap"

# ---------------------------------------------------------------------------
# Modal resources
# ---------------------------------------------------------------------------
app = modal.App("reap-prune-gpt-oss")
...
image = (
    ...
    .run_commands(
        ...
        f"sed -i 's/pairwise_distances.scatter_add_(0, idx0.unsqueeze(-1).expand(-1, E), flat_dists)/pairwise_distances.scatter_add_(0, idx0.unsqueeze(-1).expand(-1, E), flat_dists.to(pairwise_distances.dtype))/' {REAP_DIR}/src/reap/metrics.py",
...
def main(
    ...
    if observer_only:
        print("✅ Observer phase complete. Skipping pruning.")
        return
